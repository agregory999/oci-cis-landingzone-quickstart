...
locals {
  ...
  dataflow_policies_map = {
    "DATAFLOW-SQL-POLICY" : {
      name : "${var.service_label}-dataflow-sql-users-policy"
    ...
    "DATAFLOW-SERVICE-POLICY" : {
      name : "${var.service_label}-dataflow-service-policy"
  ...
  datacatalog_policies_map = {
    "DATACATALOG-POLICY" : {
      name : "${var.service_label}-datacatalog-policy"
      description : "Allows for Data Flow Service to operate"
      compartment_ocid : var.tenancy_ocid
      statements : [
        "allow group cloud-engineering-users to use data-catalog-family in compartment cloud-engineering-shared:data-catalog-lake"
      ]
    }
  }
